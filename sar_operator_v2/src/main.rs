...
use kube_runtime::{controller::{Action, Controller}, reflector::ObjectRef};
...
#[derive(CustomResource, Deserialize, Serialize, Clone, Debug, schemars::JsonSchema)]
...
async fn reconcile(sar_job: Arc<SarJob>, ctx: Arc<Context>) -> Result<Action> {
    ...
    if sar_job.metadata.deletion_timestamp.is_some() {
        if sar_job.metadata.finalizers.as_ref().map(|f| f.contains(&SARJOB_FINALIZER.to_string())).unwrap_or(false) {
            // Cleanup logic
            ...
            api.patch(&sar_job_name, &PatchParams::default(), &patch).await?;
        }
        ...
        "Pending" | "" => {
            ...
            match jobs_api.create(&PostParams::default(), &job_manifest).await {
                Ok(_) => {
                    ...
                    api.patch_status(&sar_job_name, &PatchParams::apply("sar-operator-v2"), &Patch::Merge(serde_json::json!({ "status": new_status }))).await?;
...
fn error_policy(_sar_job: Arc<SarJob>, error: &Error, _ctx: Arc<Context>) -> Action {
