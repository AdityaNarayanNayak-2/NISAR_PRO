...
impl EsaClient {
    ...
    pub async fn get_access_token(&self) -> Result<String, Box<dyn std::error::Error>> {
        ...
        let token = json["access_token"].as_str().ok_or("No access_token in response")?;
        
        Ok(token.to_string())
    }

    pub async fn search_scenes(&self, lat: f64, lon: f64) -> Result<Vec<SarScene>, Box<dyn std::error::Error>> {
        let token = self.get_access_token().await?;

        // Construct OData filter for spatial intersection
        // POINT(lon lat)
        let filter = format!("OData.CSC.Intersects(area=geography'SRID=4326;POINT({} {})')", lon, lat);
        
        // Add collection filter (Sentinel-1)
        let filter = format!("{} and Collection/Name eq 'SENTINEL-1'", filter);

        // Add product type filter (GRD)
        let filter = format!("{} and Attributes/OData.CSC.StringAttribute/any(i:i/Name eq 'productType' and i/Value eq 'GRD')", filter);

        let url = format!("{}/Products?$filter={}&$top=10&$orderby=ContentDate/Start desc", self.base_url, filter);
        ...
        let scenes = odata_response.value.into_iter().map(|p| SarScene {
            id: p.id,
