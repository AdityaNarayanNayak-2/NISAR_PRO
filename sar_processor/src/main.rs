...
async fn main() -> Result<()> {
    ...
    let (processor, raw_data) = if cli.synthetic {
        ...
        match ext.as_str() {
            "h5" | "hdf5" | "he5" => {
                ...
                let product = nisar_parser::parse_nisar_rslc(input, &cli.polarization)?;
                ...
                if cli.no_rcmc {
                    ...
                    let rcmc = RcmcParams::from_frequency(
                        p.center_frequency as f32,
                        7_200.0,   // nominal NISAR LEO velocity (m/s)
                        800_000.0, // nominal near range (m)
                    );
                    proc = proc.with_rcmc_params(rcmc);
                }

                // Optionally limit number of azimuth lines processed
    ...
    // ── Run RDA Pipeline ───────────────────────────────────────────────────
    info!(
        "Starting RDA pipeline on {}×{} image...",
        raw_data.nrows(),
        raw_data.ncols()
    );
    ...
    // ── Save Output ────────────────────────────────────────────────────────
    info!("Saving focused SAR image → {}", cli.output);
